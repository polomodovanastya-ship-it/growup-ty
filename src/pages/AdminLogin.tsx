    ...
    try {
      if (mode === "login") {
        ...
        toast({
          ...
          description: "Проверь почту для подтверждения, затем войди.",
        });
        setMode("login");
      }
    } catch (e) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 bg-background">
      <div className="w-full max-w-sm">
        ...
        <form
          onSubmit={submit}
          className="w-full space-y-5 rounded-2xl border border-border/60 p-6 md:p-8 bg-card"
        >
          <div>
            <h1 className="text-2xl font-bold text-foreground">Админ-панель</h1>
            <p className="text-sm text-muted-foreground mt-1">
              {mode === "login"
                ? "Войди, чтобы видеть результаты тестов"
                : mode === "signup"
                  ? "Создай аккаунт"
